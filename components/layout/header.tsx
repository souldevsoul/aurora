      ...
      <nav className="relative bg-white/10 backdrop-blur-lg mx-4 my-4 rounded-2xl border border-white/20" role="navigation" aria-label="Main navigation">
        <div className="container mx-auto px-4">
          <div className="relative flex h-20 items-center">
            {/* Logo */}
            <Link className="inline-flex items-center gap-2.5 group" href={getRouteByName('home')} aria-label="Aurora - Home">
              <div className="w-10 h-10 bg-gradient-to-br from-purple-600 to-violet-700 rounded-lg flex items-center justify-center transition-transform group-hover:scale-105">
              ...
              <span className="text-2xl font-semibold text-gray-900">Aurora</span>
            </Link>

            {/* Mobile menu button */}
            <button
              className="lg:hidden flex items-center justify-center h-10 w-10 ml-auto bg-gradient-to-b from-purple-600 to-violet-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2"
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden lg:flex items-center">
              {navigationItems.map((item, index) => (
                <Link
                  key={item.to}
                  className={`inline-block text-gray-900 hover:text-purple-600 ${index < navigationItems.length - 1 ? 'mr-10' : ''} font-medium transition duration-200 focus:outline-none focus:underline ${isActiveRoute(item.to) ? 'text-purple-600' : ''}`}
                  href={item.to}
            ...
            <div className="hidden lg:block ml-auto">
              <Link
                className="inline-flex items-center justify-center h-10 mr-4 px-4 text-center leading-loose text-sm text-gray-900 hover:text-purple-600 font-semibold transition duration-200 focus:outline-none focus:underline"
                href={getRouteByName('login')}
              >
                Login
              </Link>
              <Link
                className="inline-flex items-center justify-center h-10 px-4 text-center leading-loose text-sm text-white font-semibold border border-purple-600 bg-purple-600 hover:bg-purple-700 shadow-sm rounded-lg transition duration-200 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2"
          ...
          <nav className="relative flex flex-col py-6 px-10 w-full h-full bg-white overflow-y-auto" role="navigation">
            ...
            <div className="flex mb-auto items-center">
              ...
              >
                ...
                <span className="text-2xl font-semibold text-gray-800">Aurora</span>
              </Link>
              <button
